...
function makeDocsPathMap(): Map<string, string> {
  return new Map([["docs", "docs"]]);
}
...
describe("startReviewService", () => {
  ...
  it("calls agentCommand function with reviewsDir and docsDirs", async () => {
    ...
    // makeDocsPathMap has docs → "docs", so docsDirs should include siteDir/docs
