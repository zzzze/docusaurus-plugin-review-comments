...
export function createReviewsMiddleware(
  ...
  if (getPrompt) {
    app.get("/api/reviews/prompt", async (req, res) => {
      try {
        ...
        logger.error`[review-service] GET /api/reviews/prompt failed: ${err instanceof Error ? err.message : String(err)}`;
  ...
  if (getGlobalPrompt) {
    app.get("/api/reviews/global-prompt", async (_req, res) => {
      try {
        ...
        logger.error`[review-service] GET /api/reviews/global-prompt failed: ${err instanceof Error ? err.message : String(err)}`;
        res.status(500).json({ error: "Internal server error" });
      }
    });
  }
  ...
  app.get("/api/reviews/pending", async (_req, res) => {
    try {
      ...
      logger.error`[review-service] GET /api/reviews/pending failed: ${err instanceof Error ? err.message : String(err)}`;
      res.status(500).json({ error: "Internal server error" });
    }
  });
  ...
  app.get("/api/reviews", async (req, res) => {
    try {
      ...
      logger.error`[review-service] GET /api/reviews failed: ${err instanceof Error ? err.message : String(err)}`;
      res.status(500).json({ error: "Internal server error" });
    }
  });
  ...
  app.post("/api/reviews", async (req, res) => {
    try {
      ...
      logger.error`[review-service] POST /api/reviews failed: ${err instanceof Error ? err.message : String(err)}`;
      res.status(500).json({ error: "Internal server error" });
    }
  });
  ...
  app.patch("/api/reviews/:commentId", async (req, res) => {
    try {
      ...
      logger.error`[review-service] PATCH /api/reviews/${req.params.commentId} failed: ${err instanceof Error ? err.message : String(err)}`;
      res.status(500).json({ error: "Internal server error" });
    }
  });
  ...
  app.delete("/api/reviews/:commentId", async (req, res) => {
    try {
      ...
      logger.error`[review-service] DELETE /api/reviews/${req.params.commentId} failed: ${err instanceof Error ? err.message : String(err)}`;
      res.status(500).json({ error: "Internal server error" });
    }
  });
}
